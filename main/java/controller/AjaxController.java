...
//        return "编号"+parking_id+"的状态是："+pl.getParking_status();
//    }
//    @RequestMapping("/t2")
...
public void sendcarinfo(HttpServletResponse response,HttpServletRequest request) throws IOException {
    ...
    public String gerStartTime(int parking_id){
        Date date=userService.getStartTime(parking_id);
        return JsonUtils.getJson(date);
    }
    @RequestMapping("/getEndTime")
    public String gerEndTime(int parking_id){
        Date date=userService.getEndTime(parking_id);
        return JsonUtils.getJson(date);
    }
    ...
    public void  getTimeSendMoney(HttpServletRequest request, HttpServletResponse response)throws Exception{
        ...
        String second = (String) request.getParameter("second");this.num=Integer.parseInt(parking_id);
       ...
       int money=userService.count(Integer.parseInt(min));
        ...
        userService.addfareinfo(Integer.parseInt(parking_id));//添加费用信息到数据库System.out.println(1);
        ...
        System.out.println("停车时间:"+hour+":"+min+":"+second);
    }
    ...
    public void sendMoney(HttpServletResponse response,HttpServletRequest request) throws IOException{
       ...
       System.out.println("获取刚入库用户的车位号:"+parking_id);
        ...
        System.out.println(4);System.out.println("金额"+money);
        Map<String,Object> map= new HashMap<String,Object>();
       ...
       //将该用户信息、停车信息等历史记录添加到数据库
        ...
        hi.setStarttime(userService.getStartTime(parking_id));hi.setEndtime(userService.getEndTime(parking_id));
        hi.setCar_number(userService.getcarnumberByParkingId(parking_id));hi.setHost_name(userService.gethostnameByParkingId(parking_id));
        hi.setPhone(userService.getphoneByParkingId(parking_id));hi.setFare(userService.getfareById(parking_id));
        userService.addhistoryinfo(hi);System.out.println(6);
        //将该用户信息删除
        userService.deletefareinfo(parking_id);System.out.println(7);
